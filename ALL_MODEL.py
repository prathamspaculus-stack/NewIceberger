...
def build_features(input_date: DateType, history_df: pd.DataFrame):
    ...
    while current_date < input_date:
        ...
        next_date = current_date + pd.Timedelta(day=1)
        ...
        feat = {
            ...
            "roll_mean_7": df["call_count"].iloc[-7].mean(),
            "roll_mean_14": df["call_count"].iloc[-14].mean(),
            "dayofweek": input_date.dayofweek,
            "is_us_holiday" : is_us_holiday,
            "roll_std_7" : df["call_count"].iloc[-7].std()
        }
